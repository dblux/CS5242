...
# 以下のCNNモデルのレイヤーの中に出てくるConv2Dが畳み込み層の定義部分にあたります｡<br>プーリング層としてMaxPool2Dという関数を利用しています｡
# モデルの定義
...
# ## 畳み込み層
# ### ①output_channels(出力）, input_shape(入力）
# それぞれの出力のチャンネルの数<br>この場合32×32の行列３枚

# ### ②kernel_size
# カーネルのサイズ

# ### ③strides
# 画像からカーネルに含まれる特徴を探す際に､kernelをスライドさせる際の移動ピクセル数

# ### ④padding
# 画像の縁取り処理<br>畳み込み処理によって画像のサイズが縮小することを防ぐ

# ## プーリング層(MaxPool2D)
# 畳み込み後の出力画像を縮小する処理のことです｡これにより､画像に映る対象物の位置ずれや形の歪みにモデルが頑健になります｡この処理が特徴マップに表れる特徴の位置検出において､数ピクセル分の違いを吸収する。<br>エリア内のピクセル値の最大値を保存する<font color='red'>MaxPool2D(領域のピクセル数)</font>という手法が広く利用されています
