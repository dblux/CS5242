...
# ## 学習結果の確認
print('CNN Loss:', cnn_hist.history['loss'][-1])
print('CNN Accuracy:', cnn_hist.history['categorical_accuracy'][-1])
...
for i in range(824):
    ...
    pro_row = predicted_cnn[:,0][np.newaxis,:]
    pro_prob = np.concatenate((pro_prob, pro_row), axis=0)
    
    lig_id = predicted_cnn[:,0].argsort()[-10:][::-1] + 1
